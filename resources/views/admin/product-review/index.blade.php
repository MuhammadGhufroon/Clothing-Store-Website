...
@section('nav')
            ...
            <div class="col-12">
                <div class="card mb-4">
                    <div class="card-header pb-0">
                        <h6>Table of Product Reviews</h6>
                    </div>
                            ...
                            <table class="table align-items-center mb-0">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 text-center">Comment</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <!-- Loop through Product Reviews data -->
                                    @foreach($reviews as $index => $review)
                                    <tr>
                                        <td class="text-center">{{ $index + 1 }}</td>
                                        <td class="text-center">{{ $review->customer->name }}</td>
                                        <td class="text-center">{{ $review->product->product_name }}</td>
    ...
    <!-- Script SweetAlert2 -->
    <script>
        function confirmDelete(name, reviewId) {
            Swal.fire({
                title: 'Are you sure?',
                text: 'You are about to delete the review for ' + name + '. This action cannot be undone!',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    document.getElementById('frmDelete' + reviewId).submit();
                }
            });
        }
    </script>
